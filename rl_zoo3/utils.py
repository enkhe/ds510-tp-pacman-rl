...
class StoreDict(argparse.Action):
    ...
    def __call__(self, parser, namespace, values, option_string=None):
        ...
        for arguments in values:
            ...
            # Try to parse common Python literals/expressions while
            # gracefully handling unquoted strings like human/rgb_array.
            parsed: Any
            try:
                parsed = eval(value)
            except NameError:
                # Treat bare words (e.g., human) as strings
                parsed = value
            except SyntaxError:
                # Fallback to literal_eval for simple literals
                try:
                    parsed = ast.literal_eval(value)
                except Exception:
                    parsed = value
            except Exception:
                # Last resort: try literal_eval, else keep raw string
                try:
                    parsed = ast.literal_eval(value)
                except Exception:
                    parsed = value
